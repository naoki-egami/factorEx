...
formula_u <- as.formula(chosen.candidate ~ gender + profession + age + family.status + race + military.service +
education + abortion.stance + spending.stance + copartisan)
col <- all.vars(formula_u)[-1]
for(i in 1:length(col)){
  dfExp[ , col[i]] <- as.factor(dfExp[ , col[i]])
}
lapply(dfExp, class)
marExp <- createDist(formula_u, data  = dfExp)

dfExp_all <- dfExp[order(dfExp$pair_id), ]
dfExp_1   <- dfExp_all[dfExp_all$profile == 1, ]
dfExp_2   <- dfExp_all[dfExp_all$profile == 2, ]

apply(dfExp_1, 2, function(x) sum(is.na(x)))
dfExp_1 <- dfExp_1[is.na(dfExp_1$chosen.candidate) == FALSE, ]
keep_id <- setdiff(seq(1:ncol(dfExp_1)), which(is.element(colnames(dfExp_1),
                                                          c("information.condition", "choice.time"))))
dfExp_1 <- dfExp_1[, keep_id]
lapply(model.frame(formula_u, dfExp), class)
lapply(model.frame(formula_u, dfExp), levels)
lm1 <- lm(formula_u, data = dfExp)


formula_u <- as.formula(chosen.candidate ~ gender + profession + age + family.status + race + military.service +
                          education + abortion.stance + spending.stance + copartisan)
ameVisDem1 <- AME_estimate_full(formula = formula_u,
                                data = dfExp_1, pair = FALSE,
                                cluster = dfExp_1$id,
                                marginal_dist = marUse,
                                marginal_type = "Exp",
                                type = "No-Reg", difference = F)
ameVisDem1$AME

# race, gender and age set to always show
marDem <- marHigh <- marExp
marDem$prop <- c(0, 1, # gender
                 1, 0, # profession
                 0, 1, # age
                 1, 0, # family
                 0, 1, # race
...
marDist <- list(marExp, marDem, marHigh)
ameVis <- AME_estimate_full(formula = formula_u,
                                data = dfExp_1, pair = FALSE,
                                cluster = dfExp_1$id,
                                marginal_dist = marDist,
                                marginal_type = c("Exp", "Dem", "All"),
                                type = "No-Reg", difference = F)
ameVis$AME
...
facVars <-  c("gender", "profession", "age",
              "family.status", "race", "military.service",
              "education", "abortion.stance", "spending.stance",
              "party", "copartisan")
